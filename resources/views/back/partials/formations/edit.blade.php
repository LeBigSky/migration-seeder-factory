     ...
     <form action="/formation/update/{{ $formation->id }}" method="POST">
        ...
        <h2>MODIFIER UN BATIMENT</h2>
            <div class="mb-3">
              ...
              <input type="string" class="form-control" name="nom" aria-describedby="nom" value="{{ $formation->nom }}">
              <div id="emailHelp" class="form-text">Vos données sont entre de bonnes mains, faites nous confiance...</div>
            </div>
            <div class="mb-3">
              <label for="description" class="form-label">Description</label>
              <input type="text-area" class="form-control" name='description' value="{{ $formation->description }}">
            </div>
